...
namespace Games.Chocolate
{
	...
	public class SpriteEffect : MonoBehaviour {
		...
		public static float blurConst = 10f;
		public static float rgbSplitConst = 1f;
		public static int blurSpriteCount = 50;
		...
		private float blurDistanceConst;


		void Start ()
		{
			blurDistanceConst = Random.Range (0.2f, 1f);
		}
		...
		void Update ()
		{
			...
			distance = Mathf.Max (0f, Mathf.Sqrt(Geometry.lengthOfVector2(relativeVector2 - new Vector2(Game.reference.transform.position.x, Game.reference.transform.position.y))) / 5f - 0.2f);
			...
			if (effect == Effect.Blur)
			{
				distance *= blurConst * blurDistanceConst;
			}
			else if (effect == Effect.RGBSplit)
			{
