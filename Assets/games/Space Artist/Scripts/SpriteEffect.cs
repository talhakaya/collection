	...
	public class SpriteEffect : MonoBehaviour {
		...
		void Update ()
	    {
	        ...
	        if (effect != Effect.Shadow)
	        {
	            ...
	            distance = Mathf.Max(0f, Mathf.Sqrt(Geometry.lengthOfVector2(relativeVector2 - new Vector2(transform.position.x, transform.position.y))) / 5f - 0.2f);
	        ...
	        {
	            transform.position = transform.parent.position + Game.shadowVector * tint.shadowConst * parentShadowConst;
	            tint.spriteEffectColor = shadowColor;
	        }
		...
		public static void make(Effect newEffect, GameObject obj, bool relativeToMouse = true, bool relativeToObject = false, Transform relativeTransform = null, Vector2 relativeVector2 = default(Vector2))
		{
			...
			if (obj.GetComponent<SpriteEffect> () == null && obj.GetComponent<TintScript> () != null && obj.GetComponent<SpriteRenderer> () != null)
			{
	            ...
	            {
	                SpriteEffect tempSprite = createNewObject(obj, newEffect);
	                tempSprite.tint.spriteEffectColor = shadowColor;
	                tempSprite.parentShadowConst = obj.GetComponent<TintScript>().shadowConst;
